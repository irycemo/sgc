...
class Ventanilla extends Component
{
    ...
    public function updatedModeloEditarTipoTramite(){
        ...
        if($this->modelo_editar->tipo_tramite == 'exento'){
            ...
            if(!$this->tramiteAdicionaSelected){
                ...
                $this->dispatch('mostrarMensaje', ['error', "Para el parcial es necesario seleccione el tramite al que adiciona."]);
            ...
            if($this->servicio['porcentaje'] == 0){
                ...
                $this->dispatch('mostrarMensaje', ['error', "El servicio no aplica para porcentaje."]);
    ...
    public function agregarPredio(){
        ...
        if(($this->servicio['id'] == 7 || in_array($this->servicio['nombre'], $this->certificados_historia)) && count($this->predios) == 1){
            ...
            $this->dispatch('mostrarMensaje', ['error', "Solo es posible agregar 1 predioa historias catastrales."]);
    ...
    public function crear(){
        ...
        if(!empty($this->predios) && count($this->predios) != $this->modelo_editar->cantidad)
            $this->dispatch('mostrarMensaje', ['error', 'La cantidad y el número de predios debe ser la misma']);
