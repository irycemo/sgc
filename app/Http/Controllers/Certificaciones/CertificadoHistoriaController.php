    ...
    public function certificado($tramite_historia, $tramite_anticipo, $predio, $historia){
        ...
        /* if(auth()->user()->oficina->oficina == 101){ */
            ...
            $certificacion = Certificacion::create([
                                                        'año' => now()->format('Y'),
                                                        'tipo' => CertificacionesEnum::CERTIFICADO_HISTORIA,
                                                        'folio' => (Certificacion::where('año', now()->format('Y'))->where('tipo', CertificacionesEnum::CERTIFICADO_HISTORIA)->max('folio') ?? 0) + 1,
                                                        'cadena_original' => json_encode($object),
                                                        'cadena_encriptada' => base64_encode($firmaDirector),
                                                        'estado' => 'activo',
                                                        'predio_id' => $predio->id,
                                                        'oficina_id' => auth()->user()->oficina_id,
                                                        'tramite_id' => $tramite_historia->id,
                                                        'creado_por' => auth()->id()
                                                    ]);
