            ...
            <x-slot name="body">
                ...
                @forelse ($this->tramites as $tramite)
                    ...
                    <x-table.row wire:loading.class.delaylongest="opacity-50" wire:key="row-{{ $tramite->id }}">
                        ...
                        <x-table.cell title="ActuaAccioneslizado">
                            ...
                            <div class="ml-3 relative" x-data="{ open_drop_down:false }">
                                ...
                                <div x-cloak x-show="open_drop_down" x-on:click="open_drop_down=false" x-on:click.away="open_drop_down=false" class="z-50 origin-top-right absolute right-0 mt-2 w-48 rounded-md shadow-lg py-1 bg-white ring-1 ring-black ring-opacity-5 focus:outline-none" role="menu" aria-orientation="vertical" aria-labelledby="user-menu">
                                    ...
                                    @if(!$tramite->documento_de_pago && in_array($tramite->estado, ['nuevo', 'expirado']))
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-2">
                        ...
                        @if ($modelo_editar->avaluo_para)
                            ...
                            <div class="rounded-lg bg-gray-100 py-1 px-2">
                                ...
                                <p><strong>Avalúo para:</strong> {{ $modelo_editar->avaluo_para->label() }}</p>
                        ...
                        @if ($modelo_editar->predioAvaluo)

                            <div class="rounded-lg bg-gray-100 py-1 px-2">

                                <p><strong>Fecha de vencimiento:</strong> {{ $modelo_editar->predioAvaluo->claveCatastral() }}</p>
                    ...
                    @if($modelo_editar->ligadoA?->count())
                        ...
                        <div class="rounded-lg bg-gray-100 py-1 px-2 my-3">
                            ...
                            <div class="flex gap-2 items-center">
                                ...
                                <span>{{ $modelo_editar->ligadoA->año }}-{{ $modelo_editar->ligadoA->folio }}-{{ $modelo_editar->ligadoA->usuario }} - </span>

                                @if($modelo_editar->ligadoA->estado != 'concluido')

                                    <span
                                        wire:click="desvincularTramite({{ $modelo_editar->id }})"
                                        wire:loading.attr="disabled"
                                        wire:target="desvincularTramite({{ $modelo_editar->id }})"
                                        wire:confirm="¿Esta seguro que desea desvincular los trámites?"
            ...
            <div class="flex justify-end items-center gap-3">
                ...
                @if(!app()->isProduction() && $modelo_editar->estado == 'nuevo')
                    ...
                    <x-button-gray
                        ...
                        wire:loading.attr="disabled"
                        wire:target="simularPago({{ $modelo_editar->id }})">

                        <img wire:loading wire:target="simularPago({{ $modelo_editar->id }})" class="mx-auto h-4 mr-1" src="{{ asset('storage/img/loading3.svg') }}" alt="Loading">
                        Simular pago
