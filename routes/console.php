...
Artisan::command('migrar-avaluos', function(){
    ...
    foreach ($avaluos as $avaluo) {
        ...
        $old_avaluo = OldAvaluo::create([
            ...
            'ubicacion_en_manzana' => $referencias->where('tipo_007', "UB")->where('cven_007', $avaluo['ubic_019'],)->first() ? $referencias->where('tipo_007', "UB")->where('cven_007', $avaluo['ubic_019'],)->first()['desc_007'] : null,
            'observaciones' => trim($avaluo['obs1_019']) . trim($avaluo['obs2_019']) . trim($avaluo['obs3_019']),
            'tipo_vialidad' => $referencias->where('tipo_007', "TV")->where('cven_007', $avaluo['tvia_019'],)->first() ? $referencias->where('tipo_007', "TV")->where('cven_007', $avaluo['tvia_019'],)->first()['desc_007'] : null,
            'tipo_asentamiento' => $referencias->where('tipo_007', "AH")->where('cven_007', $avaluo['tase_019'],)->first() ? $referencias->where('tipo_007', "AH")->where('cven_007', $avaluo['tase_019'],)->first()['desc_007'] : null,
            'nombre_vialidad' => trim($avaluo['call_019']),
            'numero_exterior' => trim($avaluo['next_019']),
            'numero_exterior_2' => trim($avaluo['nex2_019']),
            'numero_adicional' => trim($avaluo['nadi_019']),
            'numero_adicional_2' => trim($avaluo['nad2_019']),
            'numero_interior' => trim($avaluo['nint_019']),
            'nombre_asentamiento' => trim($avaluo['colo_019']),
            'codigo_postal' => trim($avaluo['codp_019']),
            'lote_fraccionador' => trim($avaluo['lote_019']),
            'manzana_fraccionador' => trim($avaluo['manz_019']),
            'etapa_fraccionador' => trim($avaluo['zona_019']),
            'nombre_edificio' => trim($avaluo['nedi_019']),
            'clave_edificio' => trim($avaluo['cled_019']),
            'departamento_edificio' => trim($avaluo['ndpt_019']),
            'domicilio_notificacion' => trim($avaluo['nopo_019']) . ' ' . trim($avaluo['nomu_019']) . ' ' . trim($avaluo['cest_019']). ' ' .  trim($avaluo['codp_019']),
        ]);
